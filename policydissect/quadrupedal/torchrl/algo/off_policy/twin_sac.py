...
class TwinSAC(OffRLAlgo):
  ...
  """
    ...
    def update(self, batch):
      ...
      """
        ...
        if not self.reparameterization:
            ...
            policy_loss = (log_probs * (alpha * log_probs - log_policy_target).detach()).mean()
        else:
        ...
        if self.grad_clip:
            pf_grad_norm = torch.nn.utils.clip_grad_norm_(self.pf.parameters(), self.grad_clip)
        self.pf_optimizer.step()

        self.qf1_optimizer.zero_grad()
        qf1_loss.backward()
        if self.grad_clip:
            qf1_grad_norm = torch.nn.utils.clip_grad_norm_(self.qf1.parameters(), self.grad_clip)
        self.qf1_optimizer.step()

        self.qf2_optimizer.zero_grad()
        qf2_loss.backward()
        if self.grad_clip:
            qf2_grad_norm = torch.nn.utils.clip_grad_norm_(self.qf2.parameters(), self.grad_clip)
        self.qf2_optimizer.step()

        self.vf_optimizer.zero_grad()
        vf_loss.backward()
        if self.grad_clip:
            vf_grad_norm = torch.nn.utils.clip_grad_norm_(self.vf.parameters(), self.grad_clip)
    ...
    @property
    def networks(self):
        return [self.pf, self.qf1, self.qf2, self.vf, self.target_vf]

    @property
    def snapshot_networks(self):
        return [["pf", self.pf], ["qf1", self.qf1], ["qf2", self.qf2], ["vf", self.vf]]

    @property
    def target_networks(self):
        return [(self.vf, self.target_vf)]
