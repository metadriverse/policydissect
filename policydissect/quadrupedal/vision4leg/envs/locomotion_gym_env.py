...
class LocomotionGymEnv(gym.Env):
    ...
    """
        ...
        self.mass_reset = mass_reset
        self._record_video = record_video
        # self.model_condition_dim = 3
    ...
    def augment_observation_space_with_randomizer(self):
        ...
        # temp_high = self.observation_space.high
        # temp_low = self.observation_space.low
        for r in self._env_randomizers:
            ...
            # temp_high = np.concatenate([temp_high, r.upper])
            # temp_low = np.concatenate([temp_low, r.lower])
        self.model_condition_dim = 3
    ...
    """
        ...
        def change_mass(lid_reset):
            scale = self.mass_reset
            old_mass = self.robot._pybullet_client.getDynamicsInfo(self.robot.quadruped, lid_reset)[0]
            self.robot._pybullet_client.changeDynamics(self.robot.quadruped, lid_reset, mass=old_mass * scale)

        for lid in self.robot._hip_link_ids[1:]:
            change_mass(lid)

        for lid in self.robot._leg_link_ids:
            change_mass(lid)
    ...
    """
        ...
        if self.get_image:
            linkstate = self.pybullet_client.getLinkState(self.robot.quadruped, 0)
            ...
            camPos = linkstate[0]
            camPos = [camPos[i] + forwardVec[i] * 0.4 for i in range(3)]

            camUpVec = [camMat[2], camMat[5], camMat[8]]
            camTarget = [camPos[i] + forwardVec[i] * 10 for i in range(3)]
            viewMat = self.pybullet_client.computeViewMatrix(camPos, camTarget, camUpVec)

            image = self.pybullet_client.getCameraImage(400, 400, viewMat, proj_mat)
