...
class LocomotionGymEnv(gym.Env):
  ...
  def reset(self,
    ...
    def change_mass(lid_reset):
      ...
      old_mass = self.robot._pybullet_client.getDynamicsInfo(
        self.robot.quadruped, lid_reset)[0]
      self.robot._pybullet_client.changeDynamics(
        self.robot.quadruped, lid_reset, mass=old_mass * scale)
  ...
  def _get_observation(self):
    ...
    if self.get_image:
      ...
      viewMat = self.pybullet_client.computeViewMatrix(
        camPos, camTarget, camUpVec)

      image = self.pybullet_client.getCameraImage(
        400, 400, viewMat, proj_mat)
