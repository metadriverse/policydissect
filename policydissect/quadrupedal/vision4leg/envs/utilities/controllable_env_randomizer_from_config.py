...
class ControllableEnvRandomizerFromConfig(
  ...
  def _build_randomization_function_dict(self, env):
    ...
    func_dict["pd control"] = functools.partial(
      self._randomize_pd_control, minitaur=robot)
    return func_dict
  ...
  def _randomize_individual_inertia(self,
    ...
    if self._verbose:
      tf.logging.info("base inertia is: {}".format(
        randomized_base_inertia))
    ...
    if self._verbose:
      tf.logging.info("leg inertia is: {}".format(
        randomized_leg_inertia))
  ...
  def _randomize_latency(self,
    ...
    if self._fixed_delay_observation:
      sample = self._np_random.uniform(self._param_bounds[1],
                                       self._param_bounds[1] + 1e-5)
    ...
    if self._verbose:
      tf.logging.info(
        "control latency is: {}".format(randomized_latency))
  ...
  def _randomize_motor_friction(self,
    ...
    if self._verbose:
      tf.logging.info("motor friction is: {}".format(
        randomized_motor_damping))
  ...
  def _randomize_contact_restitution(self,
    ...
    if self._verbose:
      tf.logging.info("foot restitution is: {}".format(
        randomized_restitution))
  ...
  def _randomize_contact_friction(self,
    ...
    if self._verbose:
      tf.logging.info("foot friction is: {}".format(
        randomized_foot_friction))
  ...
  def _randomize_pd_control(self,
                            minitaur,
                            lower_bound,
                            upper_bound,
                            parameters=None):
    if parameters is None:
      p_sample = self._np_random.uniform(lower_bound[0],
                                         upper_bound[0])
      d_sample = self._np_random.uniform(lower_bound[1],
                                         upper_bound[1])
    else:
      p_sample, d_sample = parameters
    self._randomization_param_value_dict["pd control"] = (
      p_sample, d_sample)

    minitaur.SetMotorGains(p_sample, d_sample)
    if self._verbose:
      tf.logging.info(
        "p_gain is: {}, d_gain is: {}".format(p_sample, d_sample))
