...
from policydissect.quadrupedal.vision4leg.robots import kinematics
from policydissect.quadrupedal.vision4leg.robots import action_filter
from policydissect.quadrupedal.vision4leg.robots import robot_config
from policydissect.quadrupedal.vision4leg.robots import minitaur_motor
from policydissect.quadrupedal.vision4leg.robots import minitaur_constants
import numpy as np
import re
import math
import copy
...
MOTOR_NAMES = [
    "motor_front_leftL_joint", "motor_front_leftR_joint", "motor_back_leftL_joint", "motor_back_leftR_joint",
...
class Minitaur(object):
    ...
    """
        self.reset_position_random_range = reset_position_random_range
    ...
    def _StepInternal(self, action, motor_control_mode):
        ...
        # RECORD_DICT["joint_angle"].append(np.array(base_pos))
        # np.save("trajectory", RECORD_DICT)
        # print(RECORD_DICT)
        # if self._is_render:
        ...
        if reload_urdf:
            ...
            if self._on_rack:
                self.rack_constraint = (
                    self._CreateRackConstraint(self._GetDefaultInitPosition(), self._GetDefaultInitOrientation())
                )
            ...
            position[0] += (np.random.rand() * self.reset_position_random_range * 2 - self.reset_position_random_range)
            position[1] += (np.random.rand() * self.reset_position_random_range * 2 - self.reset_position_random_range)
            self._pybullet_client.resetBasePositionAndOrientation(
                self.quadruped, position, self._GetDefaultInitOrientation()
            )
    ...
    """
        ...
        for _ in range(100):
            self._StepInternal(
                [math.pi / 2] * self.num_motors, motor_control_mode=robot_config.MotorControlMode.POSITION
            )
    ...
    """
        ...
        if add_constraint:
            self._pybullet_client.createConstraint(
                self.quadruped, self._joint_name_to_id["knee_" + leg_position + "R_link"], self.quadruped,
                self._joint_name_to_id["knee_" + leg_position + "L_link"], self._pybullet_client.JOINT_POINT2POINT,
    ...
    def MapContactForceToJointTorques(self, leg_id, contact_force):
        ...
        for joint_id in range(leg_id * motors_per_leg, (leg_id + 1) * motors_per_leg):
            motor_torques[joint_id] = all_motor_torques[com_dof + joint_id] * self._motor_direction[joint_id]
    ...
    def GetFootPositionsInBaseFrame(self):
        ...
        for foot_id in self.GetFootLinkIDs():
            foot_positions.append(kinematics.link_position_in_base_frame(
                robot=self,
                link_id=foot_id,
            ))
    ...
    """
        ...
        # RECORD_DICT["torque"].append(np.array(actual_torque))
        # RECORD_DICT["target_position"].append(np.array(q))
        ...
        for motor_id, motor_torque, motor_enabled in zip(self._motor_id_list, self._applied_motor_torque,
    ...
    """
        if self._enable_action_interpolation and self._last_action is not None:
            ...
            proc_action = self._last_action + \
              lerp * (action - self._last_action)
        else:
            proc_action = action
