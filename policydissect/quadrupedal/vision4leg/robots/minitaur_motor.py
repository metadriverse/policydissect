...
class MotorModel(object):
  ...
  """
    ...
    def convert_to_torque(
        self, motor_commands, motor_angle, motor_velocity, true_motor_velocity, motor_control_mode=None
    ):
    ...
    """
        ...
        if (motor_control_mode is robot_config.MotorControlMode.TORQUE) or (motor_control_mode is
    ...
    """
        observed_torque = np.clip(
            self._torque_constant * (np.asarray(pwm) * self._voltage / self._resistance), -OBSERVED_TORQUE_LIMIT,
        ...
        voltage_net = np.clip(
            ...
            (self._torque_constant + self._viscous_damping) * np.asarray(true_motor_velocity), -VOLTAGE_CLIPPING,
            VOLTAGE_CLIPPING
        )
